...
== プラグインの追加
基本的にはAndroidアプリ、iOSアプリを作るにしろ全て、Android Studioを操作して行きます。
そのため、Flutterを使う準備など全てAndroidStudio上で行います。
...
//image[Flutter_new][プロジェクト新規作成]{
//}
 2. 「Flutter Application」を選択し、「Next」を選択します。
//image[Flutter_next][Flutterプロジェクト選択]{
//}
 3. 「Projectname」を「Flutter_test」とし、「Next」を選択します。
//image[Flutter_name][アプリケーション名入力]{
//}
 4. 「Set the package name」画面では特に変更を加えずに、「Finish」を選択します。
//image[Flutter_finish][パッケージ名選択]{
//}
//comment{
  画像が取得できないので、オンライン環境にて実施。
//}
 5. 「今開いている画面で開くかどうか」聞かれるので、「This Window」を選択します。

== エミュレータの設定（Android）
インストールが完了したら、AndroidStudioを開き、Androidシミュレータの設定を行います。
 1. 画面上部にある、「AVD Manager」のアイコンを選択します。
...
//image[iphone_menu][メニュー画面]{
//}

== エミュレータの動作確認（Android）
 1. 画面上部にある、「AVD Manager」のアイコンを選択します。
 2. 先ほど作成した「Nexus 5X」を起動させます。
 3. この状態で、アプリの実行ボタン（緑色の右矢印ボタン）を選択します。
 4. 次の画像のようにNexus5X上でサンプルのアプリケーションを実行されます。
//image[Android_sample_run][サンプルアプリ起動]{
//}
...
== エミュレータの動作確認（iOS）
 ...
 2. 「Open iOS Simulator」を選択します。
 3. この状態で、アプリの実行ボタン（緑色の右矢印ボタン）を選択します。
 4. 次の画像のようにiphone上でサンプルのアプリケーションを実行されます。
//image[iphone_sample_run][サンプルアプリ起動]{
//}

ここまで問題なく実行できれば、開発環境の構築は完了になります。
