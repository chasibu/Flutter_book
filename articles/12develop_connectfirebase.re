...
8. ターミナル上で次のコマンドを入力して、フォルダを移動します。

※手順2実施後にターミナル画面を閉じてしまった場合には、再び、プロジェクトのフォルダーに移動してから次のコマンドを入力して下さい。
//cmd{
  $ cd ios
...
2. あとの章でセキュリティの設定をしますが、いったん「テストモードで開始」を選択し、有効にするボタンをクリックします。
