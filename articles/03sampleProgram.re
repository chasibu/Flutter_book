...
この章では、サンプルコードを動かして、Flutterでアプリを開発する流れを学んでいきます。
...
Androidのエミュレータを作成できます。
「Androidエミュレータの設定」の項目で紹介します。
...
1. 実機のAndroidを開発者モードに切り替えます。切り替え方法はデバイスごとに異なるので検索して切り替えてください。
...
3. 「2」までの設定が完了した状態でPCとUSBでつなぐとこのようなダイアログが表示されます。
...
この項目は、必須項目ではないので実機がないときに行う程度で大丈夫です。
...
5. 設定の確認画面が出てくるので確認して「Finish」をクリックします。
...
==== iPhoneエミュレータの設定（Macのみ）

1. Android Studioを開き、iPhoneシミュレータの設定を行います。

画面上部にある「<No devices>」を選択し、「Open iOS Simulator」をクリックします。
